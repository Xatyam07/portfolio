...
- `src/components/sections/` — Homepage sections rendered in order: Hero → About → Projects → TechStack → Experience → Blog → Certificate → ResumeCTA → Contact
...
- `src/data/` — Portfolio content as typed TypeScript constants (personal info, projects, skills, experience, certificate, navigation, site-config). Blog data comes from `content/blog/` markdown, not `src/data/`.
